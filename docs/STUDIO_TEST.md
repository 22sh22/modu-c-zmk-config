...
1. Fn+T로 USB 출력을 선택합니다. USB와 BLE가 동시에 연결된 경우에도 Studio 연결과 출력 경로가 일치하도록 합니다.
2. PC의 Chrome/Edge에서 <https://zmk.studio/>에 접속하고 USB/Serial 연결에서 MODU 장치를 선택합니다.
   오른쪽은 Studio 연결 대상이 아닙니다.
3. 연결 후 Fn+R을 눌러 잠금을 해제합니다. 잠금 보호는 활성 상태로 유지합니다.
...
BLE로 시험할 때는 Fn+G로 BLE 출력을 선택합니다.
...
- 잠금 해제 키와 Fn 접근 경로를 모두 다른 기능으로 바꾸지 마십시오.
...
- [ ] USB에서 Studio 장치가 검색되고 Fn+R로 잠금 해제됨
